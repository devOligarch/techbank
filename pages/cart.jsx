...
function Cart() {
  ...
  const [__, _recordSale] = useMutation(RECORD_SALE);
  ...
  const handleRecordSale = (receivedData) => {
    if (receivedData?.status_code == 1) {
      let payload;
      ...
      payload = {
        ...
        saleVia: "website",
        paymentMode: receivedData?.payment_method,
        paymentTimestamp: new Date().getTime().toString(),
        paymentCode: receivedData?.confirmation_code,
        customerId: user?.id,
        paymentPhoneNumber: stkNumber,
      };
      ...
      _recordSale(payload)
        .then(({ data }, error) => {
          if (data?.recordSale && !error) {
            notifications.show({
              title: "Order placed successfully",
              icon: <IconCheck />,
              message: "View  and track orders in the orders section",
              color: "green",
            });

            router.push("/account?orders=true");
          } else {
            notifications.show({
              message: `Oops! Something happened while completing your order`,
              icon: <IconInfoCircleFilled />,
              color: "red",
            });
          }
    ...
    if (!paymentLink) {
      try {
        ...
        const interval = setInterval(async () => {
          try {
            console.log("Polling");
            const res = await fetch("/api/pesapal_webhook");
            if (res.status === 200) {
              const { data } = await res.json();

              console.log(data);

              handleRecordSale(data);
                  ...
                  <div className="flex justify-between">
                    <div className="mt-1">
                      ...
                      <h1 className="font-semibold">
                        Ksh. {getTotalCartPrice().toLocaleString("en-US")}{" "}
                      </h1>
                    </div>
...
const CompleteCart = () => {
  return (
    <div className="p-8 space-y-8">
      <h1 className="text-[1.3rem] font-semibold">Pairs well with</h1>
...
const CartSummary = ({ clickCheckout, cart, cartTotal }) => {
